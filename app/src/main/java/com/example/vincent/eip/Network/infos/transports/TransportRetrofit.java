...
public class TransportRetrofit {
    private Activity        activity;
    private TransportsCallback mListener;

    public void login(final Context t,
                      final UserClientInfo clientInfo,
                      final TransportsCallback listener) {
        activity = (Activity) t;
        mListener = listener;
    ...
    public void tryLogin(JSONObject jsonLogin){
        ...
        RetrofitInterfaceTransport service = retrofit.create(RetrofitInterfaceTransport.class);

        Call<Transports> call = service.getTransport(createRequestBody(jsonLogin));
        call.enqueue(new Callback<Transports>() {
            @Override
            public void onResponse(Call<Transports> call, retrofit2.Response<Transports> response) {
                manageSuccessfulLogin(response);
            }

            @Override
            public void onFailure(Call<Transports> call, Throwable t) {
                t.printStackTrace();
            }
        });
    }
    ...
    private void manageSuccessfulLogin(retrofit2.Response<Transports> response) {
        Log.d("response retrofit", response.toString());
        mListener.getTransportsCallBack(response.body());
    }
}
