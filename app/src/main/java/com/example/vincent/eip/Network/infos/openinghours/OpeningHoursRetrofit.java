...
public class OpeningHoursRetrofit {
    private Activity        activity;
    private OpeningHoursCallback listener;

    public void login(final Context t,
                      final UserClientInfo clientInfo,
                      final OpeningHoursCallback opCallback) {
        activity = (Activity) t;
        listener = opCallback;
    ...
    public void tryLogin(JSONObject jsonLogin){
        ...
        RetrofitInterfaceOpeningHours service = retrofit.create(RetrofitInterfaceOpeningHours.class);

        Call<OpeningHours> call = service.getOpeningHours(createRequestBody(jsonLogin));
        call.enqueue(new Callback<OpeningHours>() {
            @Override
            public void onResponse(Call<OpeningHours> call, retrofit2.Response<OpeningHours> response) {
                manageSuccessfulLogin(response);
            }

            @Override
            public void onFailure(Call<OpeningHours> call, Throwable t) {
                t.printStackTrace();
            }
        });
    }
    ...
    private void manageSuccessfulLogin(retrofit2.Response<OpeningHours> response) {
        Log.d("response retrofit", response.toString());
        listener.getOpeningHoursCallBack(response.body());
    }
}
