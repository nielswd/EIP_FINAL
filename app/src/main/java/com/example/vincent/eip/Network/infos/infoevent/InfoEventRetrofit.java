...
public class InfoEventRetrofit {
    private Activity        activity;
    private InfoEventsCallback mListener;

    public void login(final Context t,
                      final UserClientInfo clientInfo,
                      final InfoEventsCallback listener) {
        activity = (Activity) t;
        mListener = listener;
    ...
    public void tryLogin(JSONObject jsonLogin){
        ...
        RetrofitInterfaceInfoEvent service = retrofit.create(RetrofitInterfaceInfoEvent.class);

        Call<InfoEvents> call = service.getInfoEvents(createRequestBody(jsonLogin));
        call.enqueue(new Callback<InfoEvents>() {
            @Override
            public void onResponse(Call<InfoEvents> call, retrofit2.Response<InfoEvents> response) {
                manageSuccessfulLogin(response);
            }

            @Override
            public void onFailure(Call<InfoEvents> call, Throwable t) {
                t.printStackTrace();
            }
        });
    }
    ...
    private void manageSuccessfulLogin(retrofit2.Response<InfoEvents> response) {
        Log.d("response retrofit", response.toString());
        mListener.getInfoEventsCallBack(response.body());
    }
}
